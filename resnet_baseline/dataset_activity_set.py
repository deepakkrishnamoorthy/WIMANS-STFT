...
class STFTActivitySetDataset(Dataset):
    def __init__(self, data_pd_y, data_dir, max_len=200, normalize="log_standard"):
        self.data_pd_y = data_pd_y
        self.data_dir = data_dir
        self.max_len = max_len
        self.normalize = normalize
        user_activity_labels = encode_data_y(self.data_pd_y, "activity")
        self.labels = (user_activity_labels.sum(axis=1) > 0).astype(np.float32)
    ...
    def __getitem__(self, idx):
        ...
        return torch.from_numpy(spec), torch.from_numpy(self.labels[idx]).float()
